...
def generar_reporte_completo(ruta_carpeta_csv):
    ...
    ruta_salida = os.path.join(os.path.dirname(__file__), "resumen_eda.txt")
    ...
    with open(ruta_salida, 'w', encoding='utf-8') as f:
        f.write("RESUMEN ANÁLISIS EXPLORATORIO DE DATOS\n")
        f.write("=" * 80 + "\n\n")
        f.write(f"Fecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Archivos analizados: {len(lista_analisis)}\n")
        f.write(f"Total productos: {total_productos:,}\n")
        f.write(f"Categorías: {total_categorias}\n")
        f.write(f"Fabricantes únicos: {len(todos_fabricantes)}\n")
        f.write(f"Colores únicos: {len(todos_colores)}\n\n")

        f.write("TABLA RESUMEN\n")
        f.write("-" * 80 + "\n")
        f.write(f"{'Archivo':<30} {'Filas':>10} {'Columnas':>10}\n")
        f.write("-" * 80 + "\n")
        for analisis in sorted(lista_analisis, key=lambda x: x['num_filas'], reverse=True):
            f.write(f"{analisis['nombre_archivo']:<30} {analisis['num_filas']:>10,} {analisis['num_columnas']:>10}\n")
